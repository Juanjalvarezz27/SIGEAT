...
import { Calendar, Filter, Search, ChevronDown, ChevronUp, Info } from 'lucide-react'
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 md:p-6 mb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 md:mb-6 gap-3">
        ...
        <button
          ...
          className="flex items-center justify-center sm:justify-start space-x-1 text-sm text-gray-700 px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors w-full sm:w-auto"
        >
      ...
      {infoPeriodo && (
        <div className="mb-4 md:mb-6 bg-blue-50 border border-blue-100 rounded-xl p-3 md:p-4 text-sm">
          <div className="flex items-start">
            <Info className="inline mr-2 text-blue-500 shrink-0 mt-0.5" />
      ...
      {/* Opciones Rápidas */}
      <div className="mb-6">
        <p className="text-sm font-medium text-gray-700 mb-2">Opciones rápidas</p>
        <div className="flex flex-wrap gap-2">
          {opcionesRapidas.map(o => (
            <button
              key={o.nombre}
              onClick={() => handleOpcionRapida(o)}
              className="px-3 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-100 rounded-lg text-sm font-medium text-blue-700 transition-colors duration-200 whitespace-nowrap"
            >
              {o.nombre}
            </button>
          ))}
      ...
      {mostrarFiltrosAvanzados && (
        <div className="pt-4 border-t border-gray-200">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <p className="text-sm font-medium text-gray-700 mb-2">Rango de fechas específico</p>
            ...
            <button
              type="submit"
              disabled={cargando}
              className="w-full sm:w-auto px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl flex items-center justify-center gap-2 font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
